...
class WidgetActionsReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (action) {
            AppConstants.Actions.SKIP_TO_PREVIOUS -> interactor.skipToPrevious()
            AppConstants.Actions.SKIP_TO_NEXT -> interactor.skipToNext()
            ...
            AppConstants.Actions.CHANGE_REPEAT_MODE -> interactor.changeRepeatMode()
            AppConstants.Actions.CHANGE_SHUFFLE_NODE -> interactor.changeRandomMode()
            AppConstants.Actions.REWIND -> interactor.fastSeekBackward()
            AppConstants.Actions.FAST_FORWARD -> interactor.fastSeekForward()
        }
    }
}