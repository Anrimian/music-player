...
public abstract class DiffListAdapter<T, VH extends RecyclerView.ViewHolder>
    ...
    final CustomAsyncListDiffer<T> mDiffer;
    private final AsyncListDiffer.ListListener<T> mListener = DiffListAdapter.this::onCurrentListChanged;
    ...
    @SuppressWarnings("unused")
    protected DiffListAdapter(RecyclerView recyclerView, @NonNull AsyncDifferConfig<T> config) {
        mDiffer = new CustomAsyncListDiffer<>(recyclerView, new AdapterListUpdateCallback(this), config);
        mDiffer.addListListener(mListener);
    }
